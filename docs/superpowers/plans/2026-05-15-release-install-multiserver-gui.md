...
```text
Создать private GitHub repo `mirage` в аккаунте CrazyAngelm и загрузить release artifacts? Это внешнее действие в GitHub.
```
...
```powershell
gh repo create CrazyAngelm/mirage --private --source . --remote mirage-origin --push
```
...
```powershell
gh release create v0.1.0 .\mirage\release\mirage-server-linux-amd64 .\mirage\release\install-server.sh .\mirage\release\mirage-client.exe .\mirage\release\SHA256SUMS --repo CrazyAngelm/mirage --title "Mirage v0.1.0" --notes "Release-first MVP: Ubuntu server installer, Windows portable client, multi-server GUI, and secret-safe release gate."
```
