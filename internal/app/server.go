...
func setupServer(host string, auto bool, stdout io.Writer) error {
	...
	return printSetupSummary(stdout, base, cfg.Server.Domain, link, auto)
}

func printSetupSummary(stdout io.Writer, base string, host string, link string, auto bool) error {
	if auto {
		_, err := fmt.Fprintf(stdout, "mirage server configured at %s\nServer host: %s\nClient import link: %s\nLink saved to: %s\nWarning: client import link contains client secrets; do not publish it.\n", base, host, link, filepath.Join(base, "state", "default.link"))
		return err
	}
	_, err := fmt.Fprintf(stdout, "mirage server configured at %s\nServer host: %s\nClient import link: %s\nWarning: client import link contains client secrets; do not publish it.\n", base, host, link)
	return err
