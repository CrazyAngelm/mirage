...
func TestSidebarItemsExposeWorkingTargets(t *testing.T) {
	items := sidebarItems()
	want := []struct {
		label  string
	...
	if len(items) != len(want) {
		t.Fatalf("len(sidebarItems()) = %d, want %d", len(items), len(want))
	}
	for i, item := range items {
		if item.Label != want[i].label {
			t.Errorf("sidebarItems()[%d].Label = %q, want %q", i, item.Label, want[i].label)
		}
		if item.Target != want[i].target {
			t.Errorf("sidebarItems()[%d].Target = %q, want %q", i, item.Target, want[i].target)
		}
	}
}
